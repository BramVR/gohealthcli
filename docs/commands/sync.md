...
| `--types` | string | — | comma-separated Data Types (`string`); repeatable with --plan; defaults to "steps" when neither --types nor --all is set |
...
| `--timezone` | string | — | IANA timezone for now, today, and yesterday (default UTC) |
...
| `--plan` | bool | `false` | print per-Data-Type resolved Sync Run plans without Provider, credential, or archive write effects |
