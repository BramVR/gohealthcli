...
The Project Site needs a per-subcommand reference page. The binary owns the canonical list of subcommands, short descriptions, long descriptions, and flags, so it ships a hidden `schema --json` subcommand that emits that list as a stable JSON document. The schema is emitted from a single in-binary command registry that also drives dispatch and `--help`, so those three surfaces cannot drift apart. The Project Site build script consumes the JSON and renders one `docs/commands/<name>.md` page per subcommand plus an auto-generated `docs/commands.md` index.
