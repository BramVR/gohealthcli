...
`gohealthcli sync` archives steps, heart-rate, oxygen-saturation, daily-shaped,
sleep session, and exercise session Data Points, wearable-filtered Data Points,
and explicit steps daily Rollups idempotently, `gohealthcli status` summarizes
the local Health Archive, `gohealthcli query` runs guarded read-only SQL over
the archive, `gohealthcli export daily-steps` writes CSV or JSONL from the
