...
var commands = []commandDef{
	...
	{
		Name:                 "catalog",
		Short:                "Verify the Provider catalog against Google discovery.",
		Long:                 "Compare gohealthcli's canonical Google Health Data Type catalog with the live public v4 discovery document, or with an offline document supplied through `--discovery PATH`. The command requires no config, Health Archive, Connection, credentials, or Provider account.\n\nThe stable result status is `verified`, `verified_with_known_gaps`, or `drift_detected`; unexpected drift exits nonzero. Known local Rollup-only and upstream-only raw Data Types are reported as explicit exceptions. The public discovery document does not establish exact per-Data-Type operation or filter support, so the result reports those facts as unverifiable instead of guessing.",
		PositionalArgs:       "verify",
		PositionalCompletion: valueCompletionCatalogAction,
		Flags: withCommonSubset(catalogCommonFlagNames(),
			flagSpec{Name: "discovery", Type: "string", Default: "", Usage: "read a Google Health discovery document from PATH instead of the public endpoint", ValueCompletion: valueCompletionFile},
		),
