...
func TestInitCreatesConfigAndEmptyHealthArchive(t *testing.T) {
	...
	if strings.Join(migrations, ",") != "1:initial_archive_schema,2:add_google_identity_json,3:add_source_family_filter,4:add_daily_steps_view,5:add_first_release_normalized_views,6:add_sync_cursors,7:rename_profile_snapshots_to_identity_snapshots" {
		t.Fatalf("migrations = %v, want initial + identity + source family + normalized views + sync cursors + identity_snapshots rename", migrations)
	}
