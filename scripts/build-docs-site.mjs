...
// Internal working documents (plan.md, research.md, agents/, adr/) stay in the repo, off the site.
...
const pages = allPages.filter((page) => isAllowed(page.rel));
...
function homeHero(page) {
  ...
  const quickstartPage = pageMap.get("quickstart.md");
  const installPage = pageMap.get("install.md");
  const quickstartRel = quickstartPage ? hrefToOutRel(quickstartPage.outRel, page.outRel) : null;
  const installRel = installPage ? hrefToOutRel(installPage.outRel, page.outRel) : null;
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght,SOFT@9..144,500..700,30..100&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">
    ...
    <aside class="sidebar">
      <div class="sidebar-head">
        <a class="brand" href="${hrefToOutRel("index.html", page.outRel)}" aria-label="${productName} docs home">
          ...
          <span><strong>${escapeHtml(productName)}</strong><small>Local health archive</small></span>
        </a>
        ${themeToggleHtml()}
      </div>
      <label class="search"><span>Search</span><input id="doc-search" type="search" placeholder="sync, install, doctor"></label>
...
function highlightShellLine(line) {
  ...
  working = working.replace(/\b(gohealthcli|brew|go|git|gh|make|sudo|cd|export|cat|curl|jq|ls|mv|cp|rm|mkdir|docker|tail|node|npm|pnpm|yarn)\b/g, (m) => stashAdd(m, "hl-cmd"));
