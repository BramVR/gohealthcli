...
// (or to the relevant command page for filter/concept pills). Adding a new
// pill: pick a Material Symbols icon name (rendered via iconSvg) and point
// `href` at the right anchor.
const capabilities = [
  { name: "Steps",       icon: "directions_walk", href: "data-types.html#steps" },
  { name: "Heart Rate",  icon: "favorite",        href: "data-types.html#heart-rate" },
  { name: "HRV",         icon: "monitor_heart",   href: "data-types.html#heart-rate-variability" },
  { name: "ECG",         icon: "ecg_heart",       href: "data-types.html#electrocardiogram" },
  { name: "Rhythm",      icon: "vital_signs",     href: "data-types.html#irregular-rhythm-notification" },
  { name: "Sleep",       icon: "bedtime",         href: "data-types.html#sleep" },
  { name: "Respiratory", icon: "air",             href: "data-types.html#daily-respiratory-rate" },
  { name: "Exercise",    icon: "fitness_center",  href: "data-types.html#exercise" },
  { name: "Hydration",   icon: "water_drop",      href: "data-types.html#hydration-log" },
  { name: "Weight",      icon: "monitor_weight",  href: "data-types.html#weight" },
  { name: "Body Fat",    icon: "scale",           href: "data-types.html#body-fat" },
  { name: "Glucose",     icon: "glucose",         href: "data-types.html#blood-glucose" },
  { name: "Oxygen",      icon: "bloodtype",       href: "data-types.html#oxygen-saturation" },
  { name: "Distance",    icon: "route",           href: "data-types.html#distance" },
  { name: "Wearable",    icon: "watch",           href: "commands/sync.html" },
  { name: "Rollups",     icon: "bar_chart",       href: "data-types.html#rollups" },
];

// A sections entry can list explicit markdown paths or a directory wildcard
// (path ending in "/"). Directory wildcards expand to every `.md` file directly
// inside the directory, sorted by filename. Used for the Reference section so
// the per-subcommand pages auto-generated by `make docs-commands` are picked
// up without manually maintaining a parallel list here.
const sections = expandSections([
  ["Start", ["index.md", "install.md", "quickstart.md", "backups.md", "data-types.md"]],
...
function expandSections(raw) {
  return raw.map(([name, rels]) => [name, rels.flatMap(expandSectionEntry)]);
}

function expandSectionEntry(rel) {
  if (!rel.endsWith("/")) return [rel];
  const dir = path.join(docsDir, rel.slice(0, -1));
...
function homeHero(page) {
  ...
  return `<header class="home-hero">
        ...
        <div class="home-services" aria-label="Supported capabilities">
          ${capabilities.map((c) => renderCapabilityPill(c)).join("")}
        </div>
      </header>`;
}
...
function renderCapabilityPill(c) {
  const inner = `${iconSvg(c.icon)}<span class="cap-label">${escapeHtml(c.name)}</span>`;
  if (!c.href) return `<span class="cap">${inner}</span>`;
  return `<a class="cap" href="${escapeAttr(c.href)}">${inner}</a>`;
}
